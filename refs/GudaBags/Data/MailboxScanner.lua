    ...
    if hasItem then
        ...
        for itemIndex = 1, numAttachments do
            ...
            if not itemID or not itemLink then
                local _, link = Guda.GetItemInfo(name)
                if link then
                    itemLink = link
            ...
            if itemData.link then
                local itemName, link, itemQuality, iLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice = addon.Modules.Utils:GetItemInfo(itemData.link)
                if itemName then
...
local function BuildSendMailItemData(name, texture, count, quality)
    ...
    local itemName, retLink, itemQuality, iLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture = addon.Modules.Utils:GetItemInfo(name)
    if itemName then
...
function MailboxScanner:OnAuctionBid(type, index, bid)
    ...
    if name and buyoutPrice > 0 and bid >= buyoutPrice then
        ...
        local itemName, retLink, itemQuality, iLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice = addon.Modules.Utils:GetItemInfo(link or name)
        if itemName then
            itemData.link = retLink or itemData.link
            itemData.itemID = addon.Modules.Utils:ExtractItemID(itemData.link) or itemData.itemID
            itemData.quality = itemQuality or itemData.quality
