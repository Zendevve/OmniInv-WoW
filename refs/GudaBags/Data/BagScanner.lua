...
function BagScanner:ScanSlot(bagID, slot)
    ...
    -- Get item info (modern field order after ClientCompat normalization)
    local name, link, itemQuality, iLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice
    if itemLink then
        name, link, itemQuality, iLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice = addon.Modules.Utils:GetItemInfo(itemLink)
    end
    ...
    itemData.class = itemType
    -- Override misclassified Quest items (e.g. Juju consumables) to Consumable
    if itemType == "Quest" and itemLink and addon.Constants and addon.Constants.QUEST_CATEGORY_EXCLUSIONS then
...
function BagScanner:Initialize()
    ...
    eventFrame:SetScript("OnEvent", function()
        ...
        -- Ascension Epoch occasionally fires BAG_UPDATE with bag IDs outside
        -- the valid player/bank/keyring range (e.g. -4). Ignore those so the
        -- scanner doesn't later trip on ContainerIDToInventoryID.
        local bagID = tonumber(arg1)
        if bagID and (bagID == -2 or bagID == -1 or (bagID >= 0 and bagID <= 11)) then
            if not dirtySlots[bagID] then
                dirtySlots[bagID] = {}
            end
            if bagCache and bagCache[bagID] then
                bagCache[bagID] = nil
            end
        end
    end)
